...
/**
 * Turns a {@link ContractAssembly} into the flat {@code name -> text} map the
 * templates are written against.
 *
 * Every key here is backed by a field that actually exists in this CRM. There is
 * no {@code {{customerVatNumber}}} or {@code {{governingLaw}}}, because nothing
 * in the schema could fill them and a template placeholder that always renders
 * empty is worse than no clause at all.
 *
...
public final class ContractPlaceholders {
    ...
    public static Map<String, String> build(Contract contract, ContractAssembly assembly) {
        ...
        // --- The contract itself ---
        values.put("contractId", nullToEmpty(contract.getContractNumber()));
        values.put("contractNumber", nullToEmpty(contract.getContractNumber()));
        values.put("contractDate", DATE.format(LocalDate.now()));
        values.put("contractType", contract.getContractType().name());
        values.put("contractTypeName", contract.getContractType().displayName());

        // --- Opportunity ---
        values.put("opportunityId", nullToEmpty(deal.getOpportunityId()));
        values.put("dealName", nullToEmpty(deal.getName()));
        // The engagement's name is the closest thing the CRM has to a service
        ...
        values.put("contractStartDate", formatDate(assembly.startDate()));
        values.put("contractEndDate", formatDate(assembly.endDate()));
        values.put("termMonths", String.valueOf(assembly.termMonths()));
     ...
     * call because the class is not thread-safe and contract generation runs on
     * request threads.
     */
