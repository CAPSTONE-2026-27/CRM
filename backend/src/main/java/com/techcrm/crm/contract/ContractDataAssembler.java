...
/**
 * Fetches and validates everything a contract needs, in one pass.
 *
 * This is where "the deal is not contractable" gets decided. Nothing downstream
 * re-checks: a {@link ContractAssembly} coming out of here is complete by
 * construction, so the document renderer never has to decide what to do about a
 * missing customer address.
 *
 * Failures use the same convention as the rest of the CRM — a
 * {@code ResponseStatusException} that {@code ApiExceptionHandler} turns into
 * an {@code {"error": "..."}} body — with 404 for a deal that isn't there, 409
 * for a deal that exists but isn't ready, and 400 for one whose data is
 * incomplete. That split matters to the automation platform: a 409 is worth
 * retrying once the rep moves the stage, a 400 never is.
 */
@Service
public class ContractDataAssembler {
    ...
    public ContractDataAssembler(DealRepository dealRepository,
    ...
    /**
     * Loads the deal, or 404s. Separate from {@link #assemble} so the
     * idempotency check can run against a known-good deal before the much more
     * expensive validation does.
     */
    ...
    @Transactional(readOnly = true)
    public ContractAssembly assemble(AuthenticatedUser caller, Deal deal, GenerateContractRequest request) {
        ...
        if (isBlank(account.getBillingAddress())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Account \"" + account.getName() + "\" has no billing address. "
                            + "A contract cannot be addressed without one.");
        }

        Contact contact = resolveContact(caller, account, request.contactId());
        User owner = resolveOwner(caller, deal);

        List<ContractAssembly.ResolvedLineItem> lineItems = resolveLineItems(deal);
        ...
        LocalDate endDate = request.endDate() != null
                ? request.endDate()
                : startDate.plusMonths(properties.getDefaultTermMonths());
        ...
        return new ContractAssembly(
                ...
                deal.getCurrency(), totalAmount, startDate, endDate,
     ...
     * The stage gate.
     *
     * The workflow this serves is triggered by "Proposal Accepted", a stage this
     * CRM does not have. {@code contract.eligible-stages} names the stages that
     * mean the same thing here, rather than adding a ninth value to
     * {@link DealStages} — which the frontend pipeline board also reads, and
     * which would re-bucket every existing deal.
     */
    private void requireEligibleStage(Deal deal) {
        ...
        if (!eligible.contains(deal.getStage())) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "Deal is in stage " + deal.getStage() + " and is not ready for a contract. "
        ...
        for (String raw : properties.getEligibleStages()) {
            ...
            if (!DealStages.ALL.contains(stage)) {
                throw new IllegalStateException(
                        "contract.eligible-stages contains \"" + raw + "\", which is not a deal stage. "
                                + "Valid stages: " + String.join(", ", DealStages.ORDERED));
            }
            stages.add(stage);
        }
        if (stages.isEmpty()) {
            throw new IllegalStateException("contract.eligible-stages is empty - no deal could ever be contracted");
        }
        return stages;
    }

    /**
     * Picks the person who signs.
     *
     * An explicit contactId wins. Otherwise the account's primary contact, and
     * failing that its oldest — in both cases only one that has an email
     * address, because a contract with no route to a signer cannot progress past
     * generation.
     */
        ...
        return contactRepository.findForAccount(account.getId(), caller.organizationId()).stream()
                ...
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
                        "Account \"" + account.getName() + "\" has no contact with an email address. "
                                + "Add one before generating a contract."));
    }

    /**
     * The sales executive named in the document, and the CRM-side owner of the
     * signed result. A deal nobody owns cannot produce a contract that names its
     * counterparty.
     */
    private User resolveOwner(AuthenticatedUser caller, Deal deal) {
        if (deal.getOwnerId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Deal has no owner. Assign a sales executive before generating a contract.");
        }
    ...
    /**
     * Builds the contract's schedule from what the CRM actually records.
     *
     * There is no product entity in this CRM and this does not invent one. The
     * only product data that exists is on the lead a deal was converted from
     * ({@code leads.product}, {@code leads.product_quantity}); when the deal did
     * not come from a lead, or that lead named no product, the schedule is a
     * single line for the deal itself. Either way the money is the deal's own
     * value, which is the CRM's authoritative number.
     *
     ...
     * can leave the contract a few paise under the deal value; a document whose
     * own columns do not sum is the worse failure.
     */
    private List<ContractAssembly.ResolvedLineItem> resolveLineItems(Deal deal) {
        BigDecimal value = deal.getValue();
        if (value == null || value.compareTo(BigDecimal.ZERO) <= 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Deal value must be greater than zero to generate a contract");
        }
        ...
        if (product == null) {
            ...
            return List.of(new ContractAssembly.ResolvedLineItem(
                    1, deal.getName(), BigDecimal.ONE.setScale(2, RoundingMode.HALF_UP),
                    amount, amount, ContractLineItem.Source.DEAL_VALUE));
        }
    ...
    private String deliveryTimelineFromLead(Deal deal) {
        ...
        return leadRepository.findById(deal.getLeadId())
                ...
                .filter(t -> !isBlank(t))
                .map(t -> "Delivery expected " + t.toLowerCase() + " from the contract start date.")
                .orElse(null);
    }
